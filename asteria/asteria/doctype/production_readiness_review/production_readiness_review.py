...
class ProductionReadinessReview(Document):
	def autoname(self):
...
def create_new_version(docname):
    try:
        ...
        latest_rev = frappe.db.sql("""
            ...
            FROM `tabProduction Readiness Review`
        ...
        # Safety check
        if frappe.db.exists("Production Readiness Review", new_name):
            frappe.throw(f"Version {new_name} already exists!")

        # Deactivate all earlier versions
        frappe.db.sql("""
            UPDATE `tabProduction Readiness Review`
            SET is_active = 0
            WHERE original_reference = %s
