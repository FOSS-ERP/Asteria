...
def execute(filters=None):
	...
	if filters.get("supplier_group"):
		val = filters.get("supplier_group")
...
def validate_filters(filters, account_details):
	...
	if filters.get("supplier_group"):
		if filters.get("party_type") and filters.party_type != "Supplier":
			frappe.throw(_("Supplier Group filter can only be used with Party Type = 'Supplier'"))
...
def get_conditions(filters):
	...
	if filters.get("supplier_group"):
		# Expand to include child groups
		filters.supplier_group = get_supplier_groups_with_children(filters.supplier_group)
		conditions.append(
			"party_type = 'Supplier' AND party IN (SELECT name FROM `tabSupplier` WHERE supplier_group IN %(supplier_group)s)"
		)
...
def get_supplier_groups_with_children(supplier_groups):
    if not isinstance(supplier_groups, list):
        supplier_groups = [d.strip() for d in supplier_groups.strip().split(",") if d]

    all_groups = []
    for group in supplier_groups:
        descendants = frappe.db.get_descendants("Supplier Group", group)
        all_groups.extend([group] + descendants)
